...
>(function UniversalDateTimePicker(
  ...
  useEffect(() => {
    ...
    // "".log("[DEBUG] Refs synced:", { hour: draftHour, minute: draftMinute, period: draftPeriod })
  }, [draftHour, draftMinute, draftPeriod]);

  const handleOpenChange = useCallback(
    (newOpen: boolean) => {
      // "".log("[DEBUG] handleOpenChange:", { newOpen, open, mode, refs: { h: draftHourRef.current, m: draftMinuteRef.current, p: draftPeriodRef.current } })
      if (!newOpen && open) {
        // Popover is closing - commit the draft time using refs to avoid stale closure
        if (mode === "time" || mode === "datetime") {
          const newTime = to24HourFormat(
            draftHourRef.current,
            draftMinuteRef.current,
            draftPeriodRef.current,
          );
          // "".log("[DEBUG] Committing time:", newTime)
          if (mode === "time") {
            onChange?.(newTime);
          } else {
            // datetime mode
  ...
  const displayText = React.useMemo(() => {
    ...
    switch (mode) {
      case "date": {
        ...
        return date
          ? format(date, "PP")
          : placeholder || getDefaultPlaceholder(mode);
      }
      case "time":
        return (
          formatTime12Hour(extractTimeString(value)) ||
          placeholder ||
          getDefaultPlaceholder(mode)
        );
      case "datetime": {
        const date = extractDateObject(value);
        const time = extractTimeString(value);
